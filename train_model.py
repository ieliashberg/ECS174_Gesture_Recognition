...
def train_model(features_train, features_validation, labels_train, labels_validation, encoder, num_epochs):
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")   #use GPU for training if available. Otherwise CPU
    ...
    optimizer = optim.SGD(net.parameters(), lr=0.01, momentum=0.9)  # set our optimizer
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_loader:
            ...
            _, predicted = torch.max(outputs, 1)    # get the argmax of ouputs and set that to predicted
            num_correct_train += (predicted == y).sum().item()  # sum the amount of correct
        ...
        with torch.no_grad():
            for X, y in validation_loader:
                ...
                X, y = X.to(device), y.to(device)   # move inputs and labels to the actual device (gpu if using)
                outputs = net(X)    # get the 1d vector of likelihoods for each class
                _, predicted = torch.max(outputs, 1)    # take the argmax of each class for each output and set that to predicted
...
def main():
    features_train, features_validation, features_test, labels_train, labels_validation, labels_test, encoder = load_and_split("static_dataset_copy.csv")
    train_model(features_train, features_validation, labels_train, labels_validation, encoder, 50)

...
def augment_sequence(x63: np.ndarray,
                     p_rot=0.7, p_mask=0.4, p_warp=0.4, p_noise=0.8):
    """Augment a (T,63) hand landmark sequence in xyz-flat format."""
    T = x63.shape[0]
    pts = x63.reshape(T, 21, 3).astype(np.float32)
    ...
    if np.random.rand() < p_mask and T > 4:
        ...
        pts[s:s+w] = pts[s-1:s]  # FIXED: proper shape, no implicit broadcast
    ...
    if np.random.rand() < p_warp and T > 4:
        ...
        flat = pts.reshape(T, -1)  # (T,63)
        warped = np.stack([np.interp(t_new, t_orig, flat[:, d])
                           for d in range(flat.shape[1])], axis=1)  # (T',63)
        ...
        pts = np.stack([np.interp(t_back,
                                  ...
                                  warped[:, d])
                        for d in range(warped.shape[1])], axis=1)  # (T,63)
...
class NPZSequenceDatasetCPU:
    """
    Loads ALREADY-normalized gesture sequences.
    Performs only:
        - optional augment
        - optional velocity computation
        - returns (x, label, length)
    """
    ...
    def __getitem__(self, idx):
        with np.load(self.files[idx]) as npz:
            x = npz["x"].astype(np.float32)  # (T,63) normalized
            y = self.labels[idx]
